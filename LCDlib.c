...
void LCD_putc(char c)
{
    ...
    {
        case '\r':
            if ( x >= 0x00 && x <= 0x3F) //checks if '\r' is detected on first line
            {
                writeLCD(0, 0x00); // reset to start of first line
            }
            else {
                writeLCD(0, 0x40); // reset to start of second line
            }
            break;
        case '\n':
            if (x >= 0x00 && x <= 0x3F) // check if on first line when '\n' is detected
            {
                writeLCD(0, 0x40); //reposition to second line
            }
            else {
                writeLCD(0, 0x00); //reposition to first line
            }
            break;
        default:
            ...
            {
                writeLCD(0, 0xC0); // reposition to start of second line
            }
            break;
    }        
}
